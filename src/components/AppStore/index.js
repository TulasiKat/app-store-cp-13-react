...
class AppStore extends Component {
  ...
  tabChangedMain = tabId => {
    this.setState({cat: tabId})
  }

  render() {
    const {cat, searchInput} = this.state
    const searchResults = appsList.filter(each => {
      const searchedText = each.appName.toLowerCase()
      return each.category === cat && searchedText.includes(searchInput)
    })
        ...
        <ul className="tab-container">
          {tabsList.map(each => (
            <TabItem
              ...
              details={each}
              cat={cat}
              tabChangedMain={this.tabChangedMain}
            />
          ))}
        </ul>
